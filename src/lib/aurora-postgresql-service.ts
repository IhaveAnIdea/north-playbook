...
// Force mock mode for local development (disable real database entirely)
...
class PostgreSQLService {
  ...
  async getExercises(activeOnly: boolean = true): Promise<Exercise[]> {
    if (isDevMode()) {
      return devStorage.getExercises();
    }
  ...
  async getExercise(exerciseId: string): Promise<Exercise | null> {
    if (isDevMode()) {
      return devStorage.getExercise(exerciseId);
    }
  ...
  async saveExerciseResponse(response: Omit<ExerciseResponse, 'id' | 'createdAt' | 'updatedAt'>): Promise<ExerciseResponse> {
    if (isDevMode()) {
      return devStorage.saveExerciseResponse(response);
    }
  ...
  async createPlaybookEntry(entry: Omit<PlaybookEntry, 'id' | 'createdAt' | 'updatedAt'>): Promise<PlaybookEntry> {
    if (isDevMode()) {
      return devStorage.createPlaybookEntry(entry);
    }
  ...
  async getPlaybookEntries(userId: string, options: {
    ...
    if (isDevMode()) {
      let entries = await devStorage.getPlaybookEntries(userId);
      ...
      if (options.highlightsOnly) {
        entries = entries.filter(e => e.isHighlight);
      }
  ...
  async saveMediaAsset(data: {
    ...
    try {
      ...
      return this.mapMediaAsset(result.rows[0]);
    } finally {
      client.release();
    }
  }
